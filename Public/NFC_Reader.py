...
class NFC_Reader:
    def __init__(self):
        ...
        if self.hresult != SCARD_S_SUCCESS:
            raise Exception(
                "Failed to establish context:", SCardGetErrorMessage(self.hresult)
            )

        self.hresult, self.readers = SCardListReaders(self.hcontext, [])
        if self.hresult != SCARD_S_SUCCESS:
            raise Exception(
                "Failed to list readers:", SCardGetErrorMessage(self.hresult)
            )
        ...
        try:
            ...
            if self.hresult != SCARD_S_SUCCESS:
                raise Exception(
                    "Failed to connect to reader:", SCardGetErrorMessage(self.hresult)
                )
